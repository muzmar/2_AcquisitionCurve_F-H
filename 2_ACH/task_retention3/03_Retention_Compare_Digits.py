...
PATH = 'D:/Tests/____TASKs_FACEProject/___Reprogramming/vonFlorian/vonFlorian/FaceRecognitionBattery_Andrea/04/04_tasks_retention'
...
items_digits = read_stims('%s/%s/stimuli/4_Compare_Digits_%s.txt'%(PATH,LANGUAGE,LANGUAGE))
...
try:
    instr_screen_1 = visual.SimpleImageStim(exp_win, image='%s/%s/Compare_Digits_I1_%s.png'%(PATH, LANGUAGE, LANGUAGE))
    instr_screen_2 = visual.SimpleImageStim(exp_win, image='%s/%s/Compare_Digits_I2_%s.png'%(PATH, LANGUAGE, LANGUAGE))
...
def run_trials(items, block_id, block_size):
    ...
    for trial_count in xrange(1,block_size+1):
        ...
        target_left = visual.TextStim(exp_win, pos=[-200, 0], text=''.join(item[3:6]), font=font, color=-1, height=CHAR_SIZE, alignHoriz='center', wrapWidth=wrap_width, units=u'pix')
        target_right = visual.TextStim(exp_win, pos=[200, 0], text=''.join(item[6:]), font=font, color=-1, height=CHAR_SIZE, alignHoriz='center', wrapWidth=wrap_width, units=u'pix')
