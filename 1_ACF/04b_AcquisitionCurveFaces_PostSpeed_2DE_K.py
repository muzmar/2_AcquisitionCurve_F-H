...
items, trial_order, header = read_stims('%s/stimuli/Trials_AcquisitionCurve_Faces_m2.txt'%(PATH))
...
try:
    ...
    instructions.append(Image("Matrixen_Gesichter_03").buffer())
...
def run_trials(items, trial_order, practice=False):
    ...
    for i in trial_order:
        ...
        if not practice:
            item.extend([str(ans[-1]), str(match_answer(ans[-1], target_pos)), str(rt)])
