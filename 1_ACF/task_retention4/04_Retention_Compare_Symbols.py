...
items_symbols = read_stims('%s/%s/stimuli/6_Compare_Symbols_%s.txt'%(PATH,LANGUAGE,LANGUAGE))
...
try:
    instr_screen_1 = visual.SimpleImageStim(exp_win, image='%s/%s/Compare_Symbols_I1_%s.png'%(PATH, LANGUAGE, LANGUAGE))
    instr_screen_2 = visual.SimpleImageStim(exp_win, image='%s/%s/Compare_Symbols_I2_%s.png'%(PATH, LANGUAGE, LANGUAGE))
...
def run_trials(items, block_id, block_size):
    ...
    for trial_count in xrange(1,block_size+1):
        ...
        target_l1 = visual.SimpleImageStim(exp_win, image=u'%s/%s/stimuli/compare_stimuli/%s.bmp'%(PATH, LANGUAGE, item[3]), pos=[-250,0])
        target_l2 = visual.SimpleImageStim(exp_win, image=u'%s/%s/stimuli/compare_stimuli/%s.bmp'%(PATH, LANGUAGE, item[4]), pos=[-200,0])
        target_l3 = visual.SimpleImageStim(exp_win, image=u'%s/%s/stimuli/compare_stimuli/%s.bmp'%(PATH, LANGUAGE, item[5]), pos=[-150,0])
        target_r1 = visual.SimpleImageStim(exp_win, image=u'%s/%s/stimuli/compare_stimuli/%s.bmp'%(PATH, LANGUAGE, item[6]), pos=[150,0])
        target_r2 = visual.SimpleImageStim(exp_win, image=u'%s/%s/stimuli/compare_stimuli/%s.bmp'%(PATH, LANGUAGE, item[7]), pos=[200,0])
        target_r3 = visual.SimpleImageStim(exp_win, image=u'%s/%s/stimuli/compare_stimuli/%s.bmp'%(PATH, LANGUAGE, item[8]), pos=[250,0])
...
with codecs.open(output_file, 'wb', encoding="utf-8") as outfile:
    ...
    outfile.write('subject_id;trial;block;trial_id;identical;stim1_1;stim1_2;stim1_3;stim2_1;stim2_2;stim2_3;ans;correct;rt\n') #header
