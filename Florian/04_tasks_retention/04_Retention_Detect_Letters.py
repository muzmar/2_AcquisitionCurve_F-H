...
items_letters = read_stims('%s/%s/stimuli/2_Detect_Letters_%s.txt'%(PATH,LANGUAGE,LANGUAGE))
...
try:
    instr_screen_1 = visual.SimpleImageStim(exp_win, image='%s/%s/Detect_Letters_I1_%s.png'%(PATH, LANGUAGE, LANGUAGE))
    instr_screen_2 = visual.SimpleImageStim(exp_win, image='%s/%s/Detect_Letters_I2_%s.png'%(PATH, LANGUAGE, LANGUAGE))
